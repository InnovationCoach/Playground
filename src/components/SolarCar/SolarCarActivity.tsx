...
const SolarCarActivity: React.FC = () => {
  ...
  // Expose live activity context to Smart Learning Assistant
  useEffect(() => {
    if (teamData) {
      const totalKg = (teamData.weight?.total || 3337) / 1000;
      const calcRatio = (30 / totalKg).toFixed(1);
      (window as any).__CURRENT_ACTIVITY_CONTEXT__ = {
        activity: 'solar-car',
        tab: activeTab,
        progress: {
          activity: 'Solar Car Challenge',
          tab: activeTab,
          componentsSelected: teamData.components,
          totalWeight: `${teamData.weight?.total || 3337}g`,
          powerOutput: '30W',
          efficiencyRatio: `${calcRatio} W/kg`,
          totalScore: teamData.score?.total || 72,
          iterationsCount: teamData.currentVersion || 1,
          dragCoefficient: teamData.specs?.dragCoefficient || 0.05
        }
      };
    }
  }, [teamData, activeTab]);
